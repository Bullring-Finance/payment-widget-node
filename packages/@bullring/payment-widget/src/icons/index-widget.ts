...
class BullringPaymentWidget extends HTMLElement {
  ...
  private setupAttributes() {
    this.amount = Number(this.getAttribute('amount')) || 0;
  ...
  private renderLoading() {
    return `
      <div class="flex items-center justify-center space-x-2">
        ${LucideIcons.refreshCw({ className: 'w-6 h-6 animate-spin' })}
    ...
    if (this.error) {
      container.innerHTML = `
        <div class="error-content">
          ${LucideIcons.xCircle({ className: 'w-16 h-16 mx-auto mb-4 text-red-500 stroke-2' })}
          <p class="text-gray-800 text-center">${i18next.t('payment.error')}</p>
    ...
    container.innerHTML = `
      ...
      <div class="invoice-container">
        <div class="invoice-details">
          <p class="text-base mb-1 pl-3 invoice-label">${i18next.t('payment.lightning')}</p>
          <p class="invoice-value text-slate-400 text-sm truncate w-48 pl-3">${this.invoice || i18next.t('payment.noInvoice')}</p>
        </div>
        <button class="bg-white p-2 rounded-xl hover:bg-slate-200 hover:border-transparent copy-button mr-2" @click="${() => this.copyToClipboard(this.invoice)}">
      ...
      <div class="mt-8 text-center text-sm text-slate-400 footer">
        <p class="flex items-center justify-center gap-2 powered-by">
